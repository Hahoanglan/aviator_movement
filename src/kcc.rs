...
fn run_kcc(
    ...
    for mut ctx in &mut kccs {
        ...
        if ctx.state.crane_height_left.is_some() {
            ...
            // Slide runs before friction so it can control the friction scale
            // itself and so ground_move's acceleration doesn't fight the slide.
            handle_slide(&time, &move_and_slide, &mut ctx, &mut transform);
...
fn ground_move(
    ...
    ctx.velocity.y = 0.0;
    // Don't let normal acceleration fight the slide — handle_slide already
    // wrote the correct velocity into ctx.velocity.
    if ctx.input.slide_velocity == Vec3::ZERO {
...
fn handle_jump(
    ...
    // Carry slide momentum into the jump — only add the vertical impulse on top.
    if ctx.input.slide_velocity.length_squared() > 0.01 {
        ctx.velocity.x = ctx.input.slide_velocity.x;
        ctx.velocity.z = ctx.input.slide_velocity.z;
        ctx.input.slide_velocity = Vec3::ZERO;
    }
...
// ── Slide ─────────────────────────────────────────────────────────────────────
//
// Called from run_kcc AFTER handle_crouching (so crouching is already resolved)
// and BEFORE friction / ground_move, so we own the velocity for this tick.
//
// Height: we set ctx.state.crouching = true for the entire duration of the
// slide.  That makes the KCC switch to the same crouching_collider and the same
// camera view-height that a normal crouch uses — no extra config needed.
// ── Slide state machine ───────────────────────────────────────────────────────
//
//  IDLE      → SLIDING   : key held + was_sprinting + grounded + cooldown == 0
//  SLIDING   → EXHAUSTED : key released OR duration >= max OR speed < min OR airborne
//  EXHAUSTED → IDLE      : key released AND cooldown reaches 0
//
//  While SLIDING  : crouch collider active, reduced friction, slope accel, steering
//  While EXHAUSTED: key still held = regular crouch; no new slide until cooldown + key release
//
fn handle_slide(
    time: &Time,
    ...
    let is_active   = ctx.input.slide_velocity != Vec3::ZERO;
    let key_held    = ctx.input.sliding; // set by apply_slide observer each frame

    // ── Always tick the cooldown down ────────────────────────────────────────
    if ctx.input.slide_cooldown > 0.0 {
        ctx.input.slide_cooldown = (ctx.input.slide_cooldown - dt).max(0.0);
    }

    // ── ACTIVE slide ──────────────────────────────────────────────────────────
    if is_active {
        // Tick duration
        ctx.input.slide_elapsed += dt;

        let duration_expired = ctx.input.slide_elapsed >= ctx.cfg.slide_max_duration;
        let too_slow         = ctx.input.slide_velocity.length() < ctx.cfg.slide_min_speed;
        let went_airborne    = !is_grounded;
        let key_released     = !key_held;

        if key_released || duration_expired || too_slow || went_airborne {
            // ── End slide — hand momentum back ──────────────────────────────
            ctx.velocity.x = ctx.input.slide_velocity.x;
            ctx.velocity.z = ctx.input.slide_velocity.z;
            ctx.input.slide_velocity      = Vec3::ZERO;
            ctx.input.slide_elapsed       = 0.0;
            ctx.input.slide_cooldown      = ctx.cfg.slide_cooldown_duration;
            // handle_crouching will stand the player back up on the next tick
            // if the key is released; if still held they just stay crouched.
            return;
        }

        // ── Use crouch collider + camera height ──────────────────────────────
        // Flipping crouching = true makes derived.collider() return the same
        // crouching_collider used for normal crouch — identical height.
        ctx.state.crouching = true;

        // ── Slope gravity ────────────────────────────────────────────────────
        if let Some(ground_hit) = ctx.state.grounded {
            ...
            ctx.input.slide_velocity += downhill * ctx.cfg.gravity * steepness * dt;
        }

        // ── Steer toward look yaw ────────────────────────────────────────────
        let (yaw, _, _) = ctx.state.orientation.to_euler(EulerRot::YXZ);
        let look_fwd    = (Quat::from_rotation_y(yaw) * Vec3::NEG_Z).normalize();
        let speed       = ctx.input.slide_velocity.length();
        if speed > 0.001 {
            let dir     = ctx.input.slide_velocity / speed;
            let steer   = (ctx.cfg.slide_steer_rate * dt).clamp(0.0, 1.0);
            let new_dir = dir.lerp(look_fwd, steer).normalize_or_zero();
            ctx.input.slide_velocity = new_dir * speed;
        }

        // ── Reduced friction ─────────────────────────────────────────────────
        let speed = ctx.input.slide_velocity.length();
        if speed > 0.001 {
            let drop      = speed.max(ctx.cfg.stop_speed)
                * ctx.cfg.friction_hz
                * ctx.cfg.slide_friction_scale
                * dt;
            let new_speed = (speed - drop).max(0.0);
            ctx.input.slide_velocity *= new_speed / speed;
        }

        // ── Write into ctx.velocity ──────────────────────────────────────────
        // ground_move will skip its own acceleration (guarded separately).
        ctx.velocity.x = ctx.input.slide_velocity.x;
        ctx.velocity.z = ctx.input.slide_velocity.z;
        return;
    }

    // ── IDLE — check whether to START a slide ────────────────────────────────
    let can_start = key_held
        && ctx.input.was_sprinting          // must have been sprinting last frame
        && is_grounded
        && !ctx.state.crouching             // not already in a normal crouch
        && ctx.input.slide_cooldown <= 0.0; // cooldown must be finished
    ...
    let (yaw, _, _) = ctx.state.orientation.to_euler(EulerRot::YXZ);
    let flat_fwd    = (Quat::from_rotation_y(yaw) * Vec3::NEG_Z).normalize();

    let horiz_speed = Vec2::new(ctx.velocity.x, ctx.velocity.z)
        .length()
        .max(ctx.cfg.stop_speed);
    let fall_bonus  = (-ctx.velocity.y).max(0.0); // convert fall speed to forward momentum
    let start_speed = (horiz_speed + fall_bonus) * ctx.cfg.slide_boost;

    ctx.input.slide_velocity = flat_fwd * start_speed;
    ctx.velocity.y           = 0.0; // absorbed into the slide
    ctx.state.crouching      = true;

    // Write velocity immediately so the rest of this tick benefits.
    ctx.velocity.x = ctx.input.slide_velocity.x;
    ctx.velocity.z = ctx.input.slide_velocity.z;
}
...
fn handle_crouching(
    ...
    // A slide takes priority over a crouch — both share the same key so we
    // must not enter the crouched collider while the player is sliding.
    if ctx.input.sliding || ctx.input.slide_velocity != Vec3::ZERO {
        // If we were already crouching before the slide started, stand back up.
        if ctx.state.crouching {
            ctx.state.crouching = false;
            // Only force-stand if there's actually room; otherwise leave crouching.
