...
pub struct AccumulatedInput {
    ...
    pub sprinting: bool,
    // Whether the player was sprinting in the previous fixed update tick
    pub was_sprinting: bool,
    // Whether any frame since the last fixed update loop input a slide
    pub sliding: bool,
    pub slide_velocity: Vec3,
    /// Seconds the current slide has been active. Resets to 0 on slide start/end.
    pub slide_elapsed: f32,
    /// Cooldown remaining before a new slide can start. Counts down each tick.
...
fn apply_sprint(sprint: On<Fire<Sprint>>, mut accumulated_inputs: Query<&mut AccumulatedInput>) {
    if let Ok(mut input) = accumulated_inputs.get_mut(sprint.context) {
...
fn apply_slide(slide: On<Fire<Slide>>, mut accumulated_inputs: Query<&mut AccumulatedInput>) {
    if let Ok(mut input) = accumulated_inputs.get_mut(slide.context) {
        // Gate on was_sprinting (previous frame) so the check is independent of
        // observer firing order — Sprint and Slide fire on the same tick when
        // both keys are held, and we can't rely on apply_sprint running first.
        if input.was_sprinting {
            input.sliding = true;
        }
    }
}
...
fn clear_accumulated_input(mut accumulated_inputs: Query<&mut AccumulatedInput>) {
    for mut accumulated_input in &mut accumulated_inputs {
        *accumulated_input = AccumulatedInput {
            ...
            crouched: default(),
            was_sprinting: accumulated_input.sprinting,
            sprinting: default(),
            sliding: default(),
            // These three persist across ticks — they track ongoing slide state.
            slide_velocity: accumulated_input.slide_velocity,
            slide_elapsed: accumulated_input.slide_elapsed,
            slide_cooldown: accumulated_input.slide_cooldown,
